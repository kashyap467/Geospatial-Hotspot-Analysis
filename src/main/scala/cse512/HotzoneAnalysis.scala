...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    // Get point count of each rectangle and sort into ascending order of count
    val hotZonesDf = spark.sql("select rectangle, count(point) as pointCount from joinResult group by rectangle order by rectangle asc")
    hotZonesDf.createOrReplaceTempView("sortedPointCountResult")

    return hotZonesDf.coalesce(1)
  }
